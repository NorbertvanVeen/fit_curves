...
INDEXSTAT =[501, 503, 506] #  's504', 's507'
MPV1, MPV2, MPV3, MPV4 = [], [], [], []
#mpv = {}

def loop_station(data):  # loop door  alle stations om de MIP value te bepalen
    for i in range(len(INDEXSTAT)):
        if i == 0:
            pulseheights = data.root.s501.events.col('pulseheights') 
            ...
            pulseheights = data.root.s506.events.col('pulseheights') 
            pulseheights = np.array(pulseheights)
            pulseheights = zip(*pulseheights)
    ...
    return mpv_array() #waardes van de mips pieken voor de platen wordt door gestuurd voor gebruik in pulseheight_data
...
def fit_gauss(ph):
    for i in range(4):
        ...
        if y[np.random.random_integers(200, 300)] != 0 : # ter controle of de data van de detector betrouwbaar is, aangezien plaat 4 503 geen goede data bevatte 3-01-2013
            ...
            schat = np.where((x > 100) & (x < 500)) # geeft een tweeledig array terug    
            ...
            popt, pcov = curve_fit(func1, x1, y1, [160, 250, 10] ) # werkende fit met een handmatige guess kleine verschillen. orde van 0.2 
            #popt, pcov = curve_fit(func1, x1, y1, [max_x, b,(max_x - min_x)/2] ) # fit met guess gebruikt werkt ook
            ...
            peak = -999
            print 'The data of the detector ',i + 1, 'could not be fitted to a gauss curve, MIP value is set to', peak,'.'
    
        if i == 0:
            MIP1 = peak
            MPV1.append(MIP1)  #bij herhalen  van de loop oor ander station append deze regel op de juiste manier? OK
        elif i == 1:
            MIP2 = peak
            MPV2.append(MIP2)
        elif i == 2:
...
def mpv_array():
    mpv = {}
    for idx, station in enumerate(INDEXSTAT):
        mpv[station] = [u[idx] for u in [MPV1, MPV2, MPV3, MPV4]]
    return mpv
...
if __name__ == '__main__':
    ...
    # data = tables.openFile('C:/Python/Datasets/datatest.h5', 'a')   #identiek aan onderstaande regels met with, deze sluit automatisch de datafile af
    # loop_station()
    # data.close()
    
    with tables.openFile('C:/Python/Datasets/datatest1.h5', 'a') as data:
        loop_station(data)
