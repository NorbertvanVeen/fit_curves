...
def fit_gauss(ph):
    for i in range(3):
        ...
        schat = np.where((x > 150) & (x < 420)) # geeft een tweeledig array terug    
        ...
        #popt, pcov = curve_fit(func1, x1, y1, [1, 250, 10] ) # werkende fit met een handmatige guess kleine verschillen. orde van 0.2 
...
def hist_ph(ph):
    pylab.hist(ph[0], bins=500, range=[0, 1000], histtype='step')
    pylab.hist(ph[1], bins=500, range=[0, 1000], histtype='step')
    pylab.hist(ph[2], bins=500, range=[0, 1000], histtype='step')
    pylab.hist(ph[3], bins=500, range=[0, 1000], histtype='step')
    pylab.axis('auto')
    pylab.xlabel('pulseheight (ADC)')
    pylab.ylabel('count')
    pylab.yscale('log')
    pylab.title('Pulseheight histogram (503)')
    pylab.grid(True)
    pylab.show()
...
if __name__ == '__main__':
    ...
    data = tables.openFile('C:/Python/Datasets/datatest.h5', 'a')
